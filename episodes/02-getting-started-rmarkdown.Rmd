...
<figure>
```{r quarto, echo = FALSE, fig.show = "hold", fig.alt = "A schematic representing the multi-language input (e.g. Python, R, Observable, Julia) and multi-format output (e.g., PDF, html, Word documents, and more) versatility of Quarto. Art by Allison Horst", width="100%"}
  knitr::include_graphics("fig/quarto_schematic.png")
```
...
For example, today's date is ``` ``r "r Sys.Date()"`` ```, will be rendered as: today's date is `r Sys.Date()`.
